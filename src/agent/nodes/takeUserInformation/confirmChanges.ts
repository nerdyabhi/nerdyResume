...
export async function confirmChanges(state: AgentState) {
  ...
  if (needsConfirmation) {
    ...
    console.log("📨 User responded to confirmation:", lastMessage);
...
Set isConfirming = false if they want changes.`,
    ...
    if (!interpretation.isConfirming) {
      console.log("✏️ User wants to correct something");
      return {
        isComplete: false,
        profile: undefined,
  ...
  const summaryResponse = await openAiLLM.invoke([
    {
      role: "system",
      content: `You are NerdyResume - an AI resume assistant created by @nerdyabhi (https://github.com/nerdyabhi) 🤖
